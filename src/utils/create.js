...
function create(component, props) {
    ...
    // 可以看到，extend 创建的是 Vue 构造器（直接得到根实例，类型为VueComponent），而不是我们平时常写的组件实例，所以不可以通过 new Vue({ components: testExtend }) 来直接使用，
    ...
    // 给组件实例添加销毁方法
    // 因为vue是单根的，所以可以通过vm.$children[0]获取根组建的实例, 类型为VueComponent
